    ...
    if os.path.exists(SETTINGS_FILE):
        try:
            with open(SETTINGS_FILE, "r") as f:
                lines = f.readlines()
                sens = float(lines[0].strip())
                obs_q = int(float(lines[1].strip())) if len(lines) > 1 else 6
                npc_q = int(float(lines[2].strip())) if len(lines) > 2 else 5
                return max(20.0, min(sens, 150.0)), obs_q, npc_q
        except: return 60.0, 6, 5
    return 60.0, 6, 5
...
class RollerGame:
    def __init__(self):
        ...
        self.sens_rect = pygame.Rect(SCREEN_W - 220, 20, 180, 8)
        self.obs_rect = pygame.Rect(SCREEN_W - 220, 70, 180, 8)
        self.npc_rect = pygame.Rect(SCREEN_W - 220, 120, 180, 8)
        ...
        for p in active:
            ...
            if now >= p.crash_until:
                for n in self.npcs:
                    ...
                    # RESTORED: Scoring for passing NPCs
                ...
                # RESTORED: Scoring for passing other player
                if self.num_humans == 2:
                    ...
                    if p.lane_idx != other.lane_idx and p.z > other.z and "pp" not in p.scored_ids:
                        ...
                        if "pp" in other.scored_ids: other.scored_ids.discard("pp")
    ...
    def draw_game(self):
        ...
        for obj in all_objs:
            ...
            if isinstance(obj, dict) and 'lane' in obj: 
                ...
                w = 170 * scale
                pygame.draw.rect(self.screen, obj['color'], (int(x-w/2), int(300+(300*scale)-35*scale), int(w), int(70*scale)))
            elif isinstance(obj, dict): 
                draw_tree(self.screen, SCREEN_W//2 + (obj['x']*scale), 300 + (300*scale), 220*scale)
            else: 
                x = SCREEN_W//2 + ((obj.lane_idx - 2) * 200 * scale)
                draw_bicycle(self.screen, int(x), int(300 + (300 * scale)), 130*scale, obj.color, obj.speed, obj.speed > 0.5)
        self.draw_ui_sliders()
        self.screen.blit(self.font.render(f"P1: {self.p1.score}", True, (0, 100, 0)), (25, 25))
        if self.num_humans == 2: self.screen.blit(self.font.render(f"P2: {self.p2.score}", True, (0, 0, 100)), (25, 55))

    def draw_ui_sliders(self):
        self.draw_slider(self.sens_rect, self.sensitivity, 20, 150, "SENS")
        self.draw_slider(self.obs_rect, self.obs_quantity, 1, 10, "BLOCKS")
        self.draw_slider(self.npc_rect, self.npc_quantity, 0, 11, "RANDOMS")

    def draw_slider(self, rect, val, v_min, v_max, label):
        hx = rect.left + ((val - v_min) / (v_max - v_min)) * rect.width
        pygame.draw.rect(self.screen, (70, 70, 70), rect)
        pygame.draw.rect(self.screen, (220, 220, 220), (int(hx-6), rect.top-6, 12, 20))
        self.screen.blit(self.font.render(f"{label}: {int(val)}", True, (0,0,0)), (rect.left, rect.bottom + 2))
        ...
        while True:
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT: 
                    with open(SETTINGS_FILE, "w") as f: f.write(f"{round(self.sensitivity, 2)}\n{self.obs_quantity}\n{self.npc_quantity}")
                    return
            ...
            else: self.dragging_sens = self.dragging_obs = self.dragging_npc = False
            ...
            if self.dragging_obs:
                self.obs_quantity = int(1 + ((max(self.obs_rect.left, min(mx, self.obs_rect.right)) - self.obs_rect.left) / self.obs_rect.width) * 9)
                if self.state == "PLAYING": self.setup_race()
            if self.dragging_npc:
                self.npc_quantity = int(((max(self.npc_rect.left, min(mx, self.npc_rect.right)) - self.npc_rect.left) / self.npc_rect.width) * 11)
                if self.state == "PLAYING": self.setup_race()
            if self.state == "PLAYING":
                ...
                self.screen.blit(self.font.render(f"PLAYERS: {self.num_humans}", True, (200,200,200)), (350, 250))
                self.screen.blit(self.font.render("ENTER TO RACE", True, (50,255,50)), (350, 300))
                self.draw_ui_sliders()
