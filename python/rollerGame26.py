...
class RollerGame:
    ...
    def draw_ui_sliders(self, surf):
        self.draw_slider(surf, self.sens_rect, self.sensitivity, 20, 150, "SENS")
        self.draw_slider(surf, self.obs_rect, self.obs_quantity, 1, 10, "BLOCKS")
    ...
    def run(self):
        ...
        while True:
            ...
            if self.dragging_sens: self.sensitivity = 20.0 + ((max(self.sens_rect.left, min(mx, self.sens_rect.right)) - self.sens_rect.left) / self.sens_rect.width) * 130.0
            if self.dragging_obs:
                new_obs = int(1 + ((max(self.obs_rect.left, min(mx, self.obs_rect.right)) - self.obs_rect.left) / self.obs_rect.width) * 9)
                if new_obs != self.obs_quantity: self.obs_quantity = new_obs; self.setup_race()
            if self.dragging_npc:
                new_npc = int(((max(self.npc_rect.left, min(mx, self.npc_rect.right)) - self.npc_rect.left) / self.npc_rect.width) * 11)
                if new_npc != self.npc_quantity: self.npc_quantity = new_npc; self.setup_race()
            ...
            if self.state == "PLAYING":
                ...
                self.draw_ui_sliders(self.virtual_surface)
