...
def generate_beep(frequency, duration=0.1, volume=0.1):
    """Generates a simple sine wave sound."""
    sample_rate = 44100
    n_samples = int(sample_rate * duration)
    buf = array.array('h', [0] * n_samples)
    for i in range(n_samples):
        t = float(i) / sample_rate
        # Sine wave with a quick fade out to sound like a bell
...
def play_chord(freqs, vol=0.1):
    """Plays multiple frequencies at once for a harmony."""
    for f in freqs:
...
class RollerGame:
    def __init__(self):
        ...
        # P1: C-Major chord, higher bell | P2: G-Major chord, lower bell
            ...
            if now >= p.crash_until:
                ...
                for n in self.npcs:
                    if p.lane_idx == n.lane_idx and abs(p.z - n.z) < 50:
                        ...
                        generate_beep(p.bell_pitch).play() # Play Player's Bell
                
                # Passing Player (1000 pts + Chord)
                if self.num_humans == 2:
                    ...
                    if p.z > other.z and other.name not in p.scored_ids:
                        ...
                        play_chord(p.chord_freqs) # Play Player's Harmony
