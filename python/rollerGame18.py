...
SERIAL_PORT_2 = 'COM5' 
BAUD_RATE = 9600
# Original internal resolution for coordinate math
...
class RollerGame:
    def __init__(self):
        pygame.init()
        # Start windowed, but allow resizing
    ...
    def draw_game(self):
        ...
        for obj in all_objs:
            ...
            if isinstance(obj, dict) and 'lane' in obj:
                ...
                draw_bicycle(surf, int(x), int(300 + (300 * scale)), 130*scale, obj.color, obj.speed > 0.5)
    ...
    def run(self):
        ...
        while True:
            now = time.time()
            # Calculate mouse relative to base resolution for slider
            screen_w, screen_h = self.screen.get_size()
            raw_mx, raw_my = pygame.mouse.get_pos()
            mx = raw_mx * (BASE_W / screen_w)
            my = raw_my * (BASE_H / screen_h)
            ...
            # Scale virtual surface to physical screen
            scaled_surf = pygame.transform.smoothscale(self.virtual_surface, (screen_w, screen_h))
