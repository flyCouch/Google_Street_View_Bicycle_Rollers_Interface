...
class RollerGame:
    def __init__(self):
        ...
        self.state = "MENU"
        self.num_humans = 1
        self.p1 = Rider((0, 255, 100), "P1")
        self.p2 = Rider((0, 150, 255), "P2")
    ...
    def update_game(self, now):
        ...
        for p in active:
            ...
            if now >= p.crash_until:
                ...
                # Passing Other Player (1000 points)
                if self.num_humans == 2:
                    other = self.p2 if p == self.p1 else self.p1
                    # We use a unique ID for the other player based on their name
                    if p.z > other.z and other.name not in p.scored_ids:
                        p.score += 1000
                        p.scored_ids.add(other.name)
                    # Remove from scored IDs if they overtake you again to allow re-scoring
        ...
        for n in self.npcs:
            ...
            if lead_z - n.z > 500: 
                ...
                # Reset scored status when NPC is recycled
