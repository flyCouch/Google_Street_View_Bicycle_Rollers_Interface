...
// in the EXACT 6-part format required by the Python script: 
// RPM,SteerX,SteerY,LeftClick,RightClick,Zoom\n
...
struct Payload {
    ...
    int leftClick;
    int rightClick;
    int zoomChange; // Added back for scroll wheel emulation
};
...
void loop() {
  ...
  if (radio.available()) {
    ...
    // Print the received data to the Serial port in the EXACT 6-part format 
    // R,X,Y,L,R,Z\n
    ...
    Serial.print(",");
    Serial.println(receivedData.zoomChange); // Print the zoom change value
  }
